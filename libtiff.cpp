                               ...
                               size_t *decoded_allocated, cudaStream_t stream)
{
        ...
        if (log_level > 1) {
                fprintf(stderr,
                        "TIFF file %s: %" PRIu32 "x%" PRIu32 " %" PRIu32
                        " bits, %" PRIu32 " sample(s)\n",
                        fname, image_info->image_width,
                        image_info->image_height,
                        image_info->bits_per_sample[0],
                        image_info->samples_per_pixel);
        }
        const size_t read_size = sizeof(uint32_t) * image_info->image_width *
                                 image_info->image_height;
        ...
        const int rc = TIFFReadRGBAImageOriented(
            tif, image_info->image_width, image_info->image_height,
