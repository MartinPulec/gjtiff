...
static double normalize_coords(struct coordinate coords[4])
{
        ...
        for (unsigned i = 0; i < 4; ++i) {
                ...
                // - lat - 0-180
                // - lon - 0-360
                coords[i].latitude += 90;
                coords[i].longitude += 180;
        ...
        double lon_lat_ratio = cos(lat_mean);
...
static void adjust_size(int *width, int *height, int comp_count) {
        ...
        WARN_MSG("[rotate] Encoding of %dx%d image would require %.2f GB (>=%g "
                 "GB), reducing ",
...
struct dec_image rotate(struct rotate_state *s, const struct dec_image *in)
{
        ...
        if (req_size >= s->output_allocated) {
                cudaFree(s->output);
                s->output = NULL;
                cudaMalloc((void **)&s->output, req_size);
